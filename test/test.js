...
betterThanBefore.setups([
  function () {
    ...
    shell.exec('git config user.email "dummy@dummy.test" && git config user.name "dummy"')
...
describe('cz-emoji preset', function () {
  it('should work if there is no semver tag', function (done) {
    ...
    conventionalChangelogCore({
      ...
      .pipe(through(function (chunk) {
        ...
        expect(chunk).to.include(', closes [#1](https://github.com/epoweripione/conventional-changelog-cz-emoji-config/issues/1) [#2](https://github.com/epoweripione/conventional-changelog-cz-emoji-config/issues/2)')
  ...
  it('should replace #[0-9]+ with GitHub issue URL', function (done) {
    ...
    conventionalChangelogCore({
      ...
      .pipe(through(function (chunk) {
        chunk = chunk.toString()
        expect(chunk).to.include('[#133](https://github.com/epoweripione/conventional-changelog-cz-emoji-config/issues/133)')
        done()
      }))
  })
  ...
  it('should remove the issues that already appear in the subject', function (done) {
    ...
    conventionalChangelogCore({
      ...
      .pipe(through(function (chunk) {
        chunk = chunk.toString()
        expect(chunk).to.include('[#88](https://github.com/epoweripione/conventional-changelog-cz-emoji-config/issues/88)')
        expect(chunk).to.not.include('closes [#88](https://github.com/epoweripione/conventional-changelog-cz-emoji-config/issues/88)')
        done()
      }))
  })
  ...
  it('should fallback to the closest package.json when not providing a location for a package.json', function (done) {
    ...
    conventionalChangelogCore({
      ...
      .pipe(through(function (chunk, enc, cb) {
        ...
        expect(chunk).to.include('(https://github.com/epoweripione/conventional-changelog-cz-emoji-config/compare')
        expect(chunk).to.include('](https://github.com/epoweripione/conventional-changelog-cz-emoji-config/commit/')
        expect(chunk).to.include('](https://github.com/epoweripione/conventional-changelog-cz-emoji-config/issues/')
